...
export default function GuestList({
  ...
  const fetchGuests = async () => {
    ...
    try {
      ...
      setError(apiError.message || 'Failed to fetch guests')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setDeleteMessage(response.message || 'Guest deleted successfully!')
      fetchGuests()
      if (onRefresh) onRefresh()
    } catch (err) {
      setDeleteStatus('error')
      const apiError = err as ApiError
      setDeleteMessage(apiError.message || 'Failed to delete guest')
    } finally {
      setDeletingId(null)
      setTimeout(() => {
        setDeleteStatus('idle')
    ...
    <div className="space-y-4">
      ...
      {guests.length === 0 ? (
        <div className="text-center py-12 text-neutral-500">
          ...
          <p>No guests found.</p>
          <p className="text-sm mt-2">Create a new guest to get started.</p>
        </div>
      ) : (
                ...
                <tr key={guest.idpk}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      guest.guestStatus ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {guest.guestStatus ? 'Active' : 'Inactive'}
                    </span>
                  </td>
