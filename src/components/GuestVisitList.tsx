...
export default function GuestVisitList({
  ...
  const fetchVisits = async () => {
    ...
    setError(null)
    try {
      const data = await getAllGuestVisits()
      setVisits(data)
    } catch (err) {
      const apiError = err as ApiError
      setError(apiError.message || 'Failed to fetch guest visits')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this guest visit?')) {
      return
    }
    ...
    try {
      const response = await deleteGuestVisit(id)
      setDeleteStatus('success')
      setDeleteMessage(response.message || 'Guest visit deleted successfully!')
      fetchVisits()
      if (onRefresh) onRefresh()
    } catch (err) {
      setDeleteStatus('error')
      const apiError = err as ApiError
      setDeleteMessage(apiError.message || 'Failed to delete guest visit')
    } finally {
      setDeletingId(null)
      setTimeout(() => {
        setDeleteStatus('idle')
    ...
    <div className="space-y-4">
      ...
      {visits.length === 0 ? (
        <div className="text-center py-12 text-neutral-500">
          ...
          <p>No guest visits found.</p>
          <p className="text-sm mt-2">Create a new guest visit to get started.</p>
        </div>
      ) : (
                ...
                <tr key={visit.idpk}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      !visit.timeOut ? 'bg-green-100 text-green-800' : 'bg-neutral-100 text-neutral-800'
                    }`}>
                      {!visit.timeOut ? 'Active' : 'Completed'}
                    </span>
                  </td>
